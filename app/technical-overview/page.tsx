  ...
  return (
    <LegalPageLayout title="Technical overview" lastUpdated={`v${APP_VERSION}`}>
      ...
      <ul>
        ...
        <li><code>POST /api/auth/verify-code</code> — every accept/reject decision (recording a wrong guess, consuming a correct one) is a single conditional <code>updateMany</code>/<code>deleteMany</code> whose WHERE clause re-checks the 5-attempt cap against the row&apos;s true current state at that exact statement, not a value read earlier — this closes a race where a correct code arriving the instant after the cap is reached, but before cleanup runs, would otherwise still succeed. Also atomically single-use (two concurrent requests with the same correct code can&apos;t both mint a session), creates a Session row, and sets an httpOnly <code>tally_session</code> cookie. Both routes apply a per-IP throttle.</li>
