  ...
  return (
    <LegalPageLayout title="Technical overview" lastUpdated={`v${APP_VERSION}`}>
      ...
      <ul>
        ...
        <li><strong>Category</strong> — household-defined custom spending categories (in addition to the fixed built-in set in <code>src/data/categories.ts</code>, ~16 broad categories), each with its own icon/colour, unique per household by name. A keyword map (<code>src/data/categorySuggest.ts</code>) powers the &quot;Suggest: …&quot; one-click hint under the category picker. A row with <code>builtinKey</code> set is instead a per-household override that renames/recolours/re-icons that built-in — <code>getCategoryMeta()</code> merges it over the defaults (name only when it differs from canonical); delete it to reset. <code>sortOrder</code> holds the household&apos;s chosen position in the combined list (built-ins + customs); once any reorder happens (<code>POST /api/categories/reorder</code>) every category gets an explicit position, with built-ins that lacked an override row getting one created. <code>getOrderedCategories()</code> is the single ordered list every picker renders.</li>
        ...
        <li><strong>AssistantKbEntry</strong> — a household-private cache of previously-answered Tally Agent <em>help</em> questions (normalized question, answer, hit count, unique per household). A later identical or highly-similar app-usage question is served straight from here with no model call; money questions are never stored or reused. Not backed up (a rebuildable cache).</li>
        <li><strong>Attachment</strong> — a file (statement PDF/photo, receipt, policy) kept beside an Expense, Account or statement import. Bytes live in a <strong>private Vercel Blob</strong> store (gated behind <code>BLOB_READ_WRITE_TOKEN</code>); the row holds the blob pathname/URL, filename, type, size and exactly one owner FK. Served only through an authenticated <code>/api/attachments/[id]</code> route, never a direct blob URL. Type-restricted, 10&nbsp;MB per file, 250&nbsp;MB per household. Not part of backups (blobs live outside Postgres).</li>
        ...
        <li><strong>DatabaseBackup</strong> — a stored full-household JSON snapshot, created by admins or by the daily <code>cron/backup</code> job (<code>isAutomatic</code> distinguishes the two; automatic snapshots are pruned to the most recent 14 per household, manual ones never auto-pruned). <code>schemaVersion</code> (default 1, current snapshots write 3) tracks the payload&apos;s shape explicitly rather than sniffing it — version 1 is <code>{'{ accounts, goals, expenses, incomes, transfers }'}</code>; version 2 adds <code>statementImports, statementTransactions, merchantAliases, categories, budgets</code> (plus the since-removed <code>mapNodes/mapEdges</code> — the editable &quot;My map&quot; feature was dropped; old snapshots&apos; map arrays are ignored on restore); version 3 adds <code>moneyTrails</code>; version 4 adds <code>projects, projectItems, projectItemLinks</code> — every household-scoped table except AuditLog (append-only history, not recoverable state), BugReport (not financial data) and AssistantKbEntry (a rebuildable answer cache). All reads for a new snapshot run inside one transaction so it&apos;s a single consistent point in time. Restore deletes the household&apos;s rows across every backed-up table explicitly (not relying on schema cascades — StatementTransaction/StatementImport involve <code>onDelete: Cascade</code>, so deleting accounts without also explicitly restoring those tables would silently wipe statement history as a side effect), then recreates everything in dependency order, building an old-id → new-id map at each step so every cross-reference — including a statement row&apos;s matched-expense/transfer links, previously silently lost — resolves to the newly-created rows. A category field is remapped only when it&apos;s a custom category id, never a built-in key. Older backups restore exactly as before (missing tables are simply empty), and the oldest (bare Expense array) via a separate legacy branch.</li>
      </ul>
      ...
      <ul>
        ...
        <li><code>npm run rotate-key</code> (<code>scripts/rotate-encryption-key.ts</code>) re-encrypts all 8 sensitive Account fields under a new key, plus every DatabaseBackup&apos;s embedded account ciphertext (otherwise a pre-rotation snapshot becomes permanently unreadable once the old key is gone). A value is only skipped once confirmed to be encrypted with the destination key specifically — not merely &quot;in the current string format&quot;, which only identifies the format, not which key produced it.</li>
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td>Answers a plain-English question, routed to either the household&apos;s own JSON data or a static app-feature guide, never inventing data or features outside what&apos;s provided. App-usage answers are cached per household in <code>AssistantKbEntry</code> and a close repeat is served without a model call; money questions always use live data and are never cached (<code>src/lib/assistantKb.ts</code>).</td>
          </tr>
          <tr>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>expenses</code>, <code>expenses/[id]</code></td><td>CRUD for recurring/one-off bills; creating a one-off (<code>once</code>-cycle) expense runs a soft cross-table duplicate check (see §8); each returned row carries its source <code>statementImport</code> label</td></tr>
          <tr><td><code>expenses/merge</code></td><td>Merges duplicate expense records into one — re-points linked transfers, matched statement rows, merchant aliases, project links and attachments at the kept record, then deletes the rest, all in one transaction. Drives the &quot;Review &amp; merge&quot; flow on the Spending list.</td></tr>
          ...
          <tr><td><code>trails</code>, <code>trails/[id]</code></td><td>Money trails — create/list/rename/delete a named chain of transfers; <code>PATCH</code> also adds/removes member transfers (sets/clears <code>Transfer.trailId</code>)</td></tr>
          <tr><td><code>projects</code>, <code>projects/[id]</code>, <code>projects/[id]/items</code>, <code>projects/[id]/items/[itemId]</code></td><td>Home projects (Planned tab) — CRUD for a <code>Project</code>, its <code>ProjectItem</code> line items, and each item&apos;s links to real Expense/Transfer records (the item <code>PATCH</code> also takes <code>addLinks</code>/<code>removeLinkIds</code>). Actuals are computed client-side from the linked records; deleting a project never touches the linked ledger rows</td></tr>
          <tr><td><code>categories</code>, <code>categories/[id]</code>, <code>categories/reorder</code></td><td>CRUD for household categories (custom, and rename/recolour overrides for built-ins); <code>reorder</code> sets the combined list order</td></tr>
          ...
          <tr><td><code>statements/activity</code></td><td>Newest-first feed of every resolved statement row across all imports (derived from <code>StatementTransaction</code>, no separate log table) — powers the Statement activity panel; &quot;undo&quot; from there calls the per-row <code>reset</code> action below</td></tr>
          ...
          <tr><td><code>admin/backup</code></td><td>Full household snapshot/restore (admin) — every household-scoped table (see §2 data model note), with cross-reference remapping on restore</td></tr>
      ...
      <ul>
        ...
        <li><strong><code>src/components/</code></strong> — one component per feature area (e.g. AccountsSection, StatementImportModal, StatementsSection, MoneyMap, ProgressSection, OptimizationInsights, AdminSection), plus shared modals and chrome (Navbar, PrivacyBlurOverlay, HelpGuideModal, ChangelogModal). <code>src/components/tally-agent/</code> holds the floating Tally Agent (launcher + assistant panel), wired to <code>/api/assistant/ask</code> and, for feedback, <code>/api/bugs</code>.</li>
      ...
      <ul>
        ...
        <li><strong>Money trails</strong> — an optional named grouping of transfers (<code>MoneyTrail</code> + <code>Transfer.trailId</code>) that follow the same money across several hops (income landing → sweep to savings → back → pay the card), rendered as a path with a &quot;where the money sat, and for how long&quot; breakdown. Purely a lens — the member transfers still stand on their own and no total changes.</li>
        ...
        <li><strong>Progress</strong> (the tab, id <code>goals</code>) — one view for slow-moving positions: a <strong>Savings</strong> section (every <code>SAVINGS</code>/<code>CREDIT_UNION</code>/<code>INVESTMENT</code>/<code>SHARES</code>/<code>STATE_SAVINGS</code> account with a balance, grouped by vehicle with a subtotal per group, each account&apos;s share of the total, and a combined total), a <strong>Loans &amp; cards</strong> section (a payoff bar per <code>LOAN</code>/<code>CREDIT_CARD</code> account from <code>originalAmount</code> vs current <code>balance</code>, plus payments logged toward it — transfers whose <code>toAccountId</code> is that account) and a <strong>Savings goals</strong> section (progress bars, optional account link, optional link to a recurring Expense, equal-payments split calculator, and a projected finish date from the monthly-equivalent of expenses linked to the goal vs the amount remaining). No amortisation schedule — just paid-vs-remaining. A totals strip shows total owed / total saved / goals on track.</li>
        <li><strong>Files</strong> (the <code>files</code> tab, <code>FilesSection</code>) — a household-wide browser for every attachment: filter by file type, linked-record kind and date, search by filename, storage-used bar, click-through to the linked record.</li>
        <li><strong>File attachments</strong> — attach a statement PDF/photo, receipt or policy to an existing Expense, Account or statement import from its editor (<code>AttachmentStrip</code>). The original file is also auto-saved to Files during a statement import and a receipt scan. Bytes in a private Vercel Blob store; served only through an authenticated route. Type-restricted, 10&nbsp;MB per file, 250&nbsp;MB per household; not part of backups.</li>
        ...
        <li><strong>Home projects</strong> (Planned tab) — a mini project manager: a <code>Project</code> holds <code>ProjectItem</code> line items with an <em>estimated</em> cost; the <em>actual</em> is the sum of real Expense/Transfer records linked to each item (<code>ProjectItemLink</code>, cascade-deleted with the item or the record, optional <code>amountOverride</code> to split one payment across items). Per-item over/under, project rollup of spent vs budget, status (planning / active / done) and target date. Entirely separate from the ledger — a project is a lens over records that stand on their own.</li>
        <li><strong>Budgets</strong> — an optional monthly spending limit per category (built-in or custom), shown as a progress bar against that category&apos;s current monthly-equivalent spend under Spending → &quot;Understand your spending&quot; → &quot;Category limits&quot;; categories with no limit set show no bar. Deliberately lightweight: one static limit, no rollover of unused amounts, no history/trend over time, no per-member budgets.</li>
        <li><strong>Duplicate-bill cleanup</strong> — <code>src/utils/duplicateExpenses.ts</code> groups active records sharing a fingerprint (a recurring bill by name + amount + cycle + currency; a one-off by those plus its date, so repeat same-price purchases aren&apos;t flagged). The Spending list shows a &quot;Possible duplicate&quot; badge and a &quot;Duplicates&quot; filter; &quot;Review &amp; merge&quot; opens <code>MergeDuplicatesModal</code>, which calls <code>POST /api/expenses/merge</code>. On statement import, <code>findDuplicateRecurringExpense</code> (in <code>statementMatching.ts</code>) makes &quot;Add as bill&quot; offer &quot;Link to &lt;existing bill&gt;&quot; when a close match already exists, and confirming an &quot;Add as bill&quot; learns a <code>MerchantAlias</code> pointing at the new bill so the next import suggests linking.</li>
        ...
        <li><strong>Reports</strong> — a dedicated tab with six views: Trends, Category &amp; Vendor, Committed, Category trend, Timeline and Insights. All but Committed cover a selectable period (1/3/6/12 months, all time) and aggregate client-side (<code>src/utils/reports.ts</code>) from one endpoint (<code>GET /api/reports/transactions</code>) built on the <em>complete</em> Transfer ledger rather than only transfers linked to a tracked Expense/Income — a genuine accuracy fix over the Overview trend chart&apos;s narrower <code>/api/history</code> data. That endpoint fills each row&apos;s category from its linked Expense, falling back to the matched statement row&apos;s inferred category; category-less money-out (unlinked transfers) groups under &quot;Transfers out&quot;, not &quot;Uncategorized&quot;. <strong>Committed</strong> is built from the tracked Expense list instead — active bills/expenses grouped by category, one line per name (records sharing a name collapse with a &quot;×N&quot; and last-paid date), monthly or annual basis; it replaced the old &quot;By category&quot; chart on the Spending page. Every tabular report exports to CSV (<code>src/utils/reportExport.ts</code>); a print stylesheet makes the browser&apos;s native Print → Save as PDF produce a clean report with no extra dependency.</li>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>AUTH_SECRET</code></td><td>Yes, unless <code>CREDENTIALS_ENCRYPTION_KEY</code> is set</td><td>Keys the digest sign-in codes are hashed with before storage — falls back to <code>CREDENTIALS_ENCRYPTION_KEY</code>; if neither is set, code issuance fails closed rather than using a hardcoded secret</td></tr>
          <tr><td><code>GOOGLE_AI_API_KEY</code></td><td>Yes, for any AI feature</td><td>Gemini API key</td></tr>
          <tr><td><code>BLOB_READ_WRITE_TOKEN</code></td><td>Yes, for file attachments</td><td>Token for a private Vercel Blob store; without it, attachment upload/serve return 503 and nothing else is affected</td></tr>
