...
export const HELP_GUIDE_SECTIONS: HelpGuideSection[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'The Spending page opens on your Expenses list: search, choose a category, filter by status, or change the sort order without scrolling past charts first. Picking a category shows that category\'s cost at the top of the list — the monthly run-rate of its recurring bills plus the combined total of any one-off expenses in it.',
      'Paid/Unpaid on each expense tracks the current payment cycle. Expand a row to see its details and Active/Paused control; Edit, update amount, contact vendor, duplicate, and delete are grouped under the three-dot More menu. A bill created from a statement import names the import it came from ("From …") so copies from different months are easy to tell apart.',
      'If a bill or one-off looks like it was added more than once (same name, amount and cycle — usually from running "Add as bill" on each month\'s statement), the row shows a "Possible duplicate" badge and a "Duplicates" status filter appears. From there, "Review & merge" lets you keep one record per bill: its payments, matched statement rows, receipts and links move onto the one you keep and the rest are deleted.',
      'Below the expense list, "Understand your spending" keeps spending history and category limits together in one tabbed area. The full category breakdown moved to Reports → Committed.',
      ...
      'Switch to the "Income" tab to record salary, freelance or rental income and see your money in vs money out. Link each income source to the account it lands in so Money Map and money-flow analysis can use it. If the same income has been added more than once (e.g. "Add as income" run on each month\'s statement), the Income tab shows a "Review & merge" button that collapses the copies into one, moving the received-payment history onto the record you keep.',
  ...
  },
  {
    ...
    body: [
      'On the Spending page, under "Understand your spending" → "Category limits", click "Set a budget" to give any category (built-in or custom) a monthly limit.',
  ...
  },
  {
    ...
    body: [
      ...
      '"Upcoming renewals & debits" totals what\'s due in the next 7 and 30 days, then lists them chronologically — a quick way to plan cash flow around due dates without scanning the full Spending ledger. Search the schedule by name, vendor, payment method or cycle; the 7- and 30-day totals follow the search. Each row in the full schedule has edit and delete buttons.',
    ],
    screenshots: [
  ...
  },
  {
    ...
    body: [
      ...
      'The import\'s name is just a label you can edit — the reliable "when" is shown next to it: the actual date range the rows cover (from the transactions themselves) plus the statement period if Tally read one off a PDF/photo. The Statements list shows the same span under each import.',
      'If Tally also read the opening and closing balance off a PDF or photo statement, the review screen shows whether the rows it logged actually add up to that balance change — a quick sanity check that nothing was missed. Credit-card and loan statements are handled correctly here (their stated balance is what you owe, so a purchase raises it). CSV imports skip this, since a CSV export has no balance to read.',
      ...
      'Add a recurring bill from a statement ONCE. The first time a subscription or direct debit shows up, use "Add as bill" to create the recurring record. Every month after that the same charge is just that bill being paid again — use "Link to a bill" (or "Yes, that\'s right" on a suggested match), which records the payment without creating anything new. Hitting "Add as bill" each month makes duplicate bills that each add their own amount to your totals. If you do try to add a bill that already exists, the row now offers "Link to <that bill>" instead. (One-off purchases are different — "Add as expense" every time is correct there, since each really is separate.)',
      'For rows that aren\'t a bill you\'ve tracked, use "Add as expense" to log them with a proper spending category (Tally remembers the category per merchant for next time), or "Log as transfer" for a quick, uncategorised entry — those show as "Transfers out" in Reports → Category & Vendor rather than under a spending category.',
      'Money coming in shows different buttons: "Link to income" ties it to one of your recorded income sources at its real amount and date — this is how a fluctuating salary reconciles, and marks that income "Received" automatically. Use "Add as income" only the first time an income source shows up; after that link each month\'s credit to it (if you try to "Add as income" for one you already track, the row offers "Link to it" instead). Use "Log as transfer" for a one-off credit that isn\'t tracked income, like a refund or a gift.',
  ...
  },
  {
    ...
    title: 'Reports',
    body: [
      'The "Reports" tab has six views — Trends, Category & Vendor, Committed, Category trend, Timeline, and Insights. Trends, Category & Vendor, Category trend and Timeline cover a period you choose (1/3/6/12 months, or all time); Committed is a snapshot of what you\'re signed up to spend right now.',
      'Trends, Category & Vendor, Category trend and Timeline are built from the household\'s full real transfer ledger, including ad-hoc transfers not tied to a tracked bill or income, so nothing real gets missed. A transfer between two of your own accounts never counts as spend or income, the same rule Transactions and Spending use. Money out via an unlinked transfer (an ad-hoc "Log as transfer", or spend logged straight off a statement) has no category, so in "Category & Vendor" it groups under "Transfers out" rather than being dropped or mixed in with your categorised bills.',
      'Committed is different — it\'s built from your tracked bills and expenses, not the transaction ledger, so it shows where your committed money goes regardless of what\'s been imported. Each category is a bar sorted biggest first; expand one to see its bills, toggle between a monthly and an annual basis, and export to CSV. Where several records share a name it shows "×N" and the most recent payment date, so accidental duplicates stand out. This view replaced the "By category" breakdown that used to sit at the bottom of the Spending page.',
    ...
    screenshots: [
      { src: '/guide/reports.png', alt: 'Reports tab showing a Trends chart of monthly spending versus income', caption: 'Reports — Trends, Category & Vendor, Committed, Category trend, Timeline, and Insights.' },
    ],
  },
  {
    id: 'files',
  ...
  },
  {
    ...
    body: [
      ...
      'The whole screen blurs automatically for privacy after 90 seconds of inactivity (or when the tab loses focus) — click "Reveal Tally" to return, or toggle it manually anytime with the eye icon in the top bar. There\'s also a quick-hide (eye) button in the bottom-left corner that blurs the screen instantly. While the privacy screen is up the Tally Agent launcher and panel are hidden too, so no live figures are left showing over the blur.',
  ...
  },
  {
    ...
    body: [
      ...
      'What happens if you delete an account? Expenses, income, transfers and savings goals linked to it are not deleted — they just lose that account link (e.g. "paid from" becomes unset). Money Map redraws itself from whatever accounts and flows remain.',
      ...
      'Why do I have the same bill several times? Usually from clicking "Add as bill" on that merchant in each month\'s statement import instead of "Link to a bill". Each click makes a separate recurring record, and all of them count toward your totals. Fix it on the Spending page: switch to the "Duplicates" filter, then "Review & merge" to collapse each set into one (the payment history moves to the record you keep). Going forward, add a bill once and link every month after.',
  ...
  },
  {
    ...
    body: [
      ...
      'For a real, restorable backup, ask an admin to use "Admin → Database Snapshots" — it covers your entire household (accounts, goals, bills, income, transfers, statement imports and their matches, custom categories, budgets, money trails and projects), and can genuinely restore it all back to that point in time.',
