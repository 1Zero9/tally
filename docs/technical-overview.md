...
- **Category** — household-defined custom spending categories (in addition to the fixed built-in set in `src/data/categories.ts`), each with its own icon/colour, unique per household by name. A row with `builtinKey` set is instead a per-household override that renames/recolours/re-icons that built-in — `getCategoryMeta()` merges it over the defaults (name only when it differs from canonical); deleting the override row resets that built-in.
