...
- **Income** — recurring or one-off income, with frequency, optional `nextPayDate`, and a linked deposit `Account`. `amount` is the household's typical/expected figure, used everywhere as a fallback estimate — for a month where a real linked `Transfer` exists (from marking received or a statement's "Link to income"), `getIncomeMonthlyContribution()` (`src/utils/calculations.ts`) uses that real, dated, possibly-fluctuating figure instead. `isReceivedThisCycle`/`lastReceivedAt` are a single flag/timestamp per record (same limitation as `Expense.isPaidThisCycle`) — un-marking received deletes that month's real linked `Transfer`(s) too (`PUT /api/income`), so re-confirming with a corrected amount doesn't double-count.
...
- **Category** — household-defined custom spending categories (in addition to the fixed built-in set in `src/data/categories.ts`, ~16 broad categories), each with its own icon/colour, unique per household by name. A conservative keyword map (`src/data/categorySuggest.ts`, `suggestCategory()`) powers the one-click "Suggest: …" hint under the category picker in the Add-expense modal and the statement categorise flow; it also matches against custom category names. A row with `builtinKey` set is instead a per-household override that renames/recolours/re-icons that built-in — `getCategoryMeta()` merges it over the defaults (name only when it differs from canonical); deleting the override row resets that built-in. `sortOrder` holds the household's position in the one combined list (built-ins + customs); `POST /api/categories/reorder` writes an explicit position to every category (creating override rows for built-ins that lacked one), and `getOrderedCategories()` is the single ordered list every category picker renders.
...
- **DatabaseBackup** — a stored full-household JSON snapshot, created by admins or by the daily `cron/backup` job (`isAutomatic` distinguishes the two; automatic snapshots are pruned to the most recent 14 per household, manual ones never auto-pruned). `schemaVersion` (default `1`, current snapshots write `3`) tracks the payload's shape explicitly rather than sniffing it — version 1 is `{ accounts, goals, expenses, incomes, transfers }`; version 2 adds `statementImports, statementTransactions, merchantAliases, categories, budgets, mapNodes, mapEdges`; version 3 adds `moneyTrails`; version 4 adds `projects`, `projectItems`, `projectItemLinks` (restored projects → items → links, with link `expenseId`/`transferId` remapped through the same id maps, after the Expense/Transfer loops) — every household-scoped table except `AuditLog` (an append-only history, not recoverable state) and `BugReport` (not financial data). All reads for a new snapshot run inside one `$transaction` (`src/lib/backup.ts`) so it's a single consistent point in time, not up to 12 independently-timed queries. Restore deletes the household's rows across every backed-up table (explicitly, not relying on schema cascades — `MapNode`/`MapEdge` and `StatementTransaction`/`StatementImport` involve `onDelete: Cascade` from Account/StatementImport respectively, so deleting accounts without also explicitly restoring the map/statement tables would silently wipe the custom Money Map and statement history as a side effect), then recreates everything in dependency order, building an old-id → new-id map at each step so every cross-reference — including a statement row's `matchedExpenseId`/`matchedTransferId`, previously silently lost — resolves to the newly-created rows. A `category` string field (Expense/Budget/MerchantAlias/StatementTransaction.suggestedCategory) is remapped only when it's a custom category id (`isBuiltinCategory()` distinguishes a built-in key like `"utilities"`, which never needs remapping, from a household-defined one, which does). Older backups predating this shape restore exactly as before via the same code path (missing tables are simply empty), and backups older still (bare Expense array) restore via a separate legacy branch.
...
1. `POST /api/auth/send-code` — issues a `VerificationToken` (6-digit code from `crypto.randomInt`, short expiry, stored only as a keyed digest via `hashCode()` in `src/lib/otp.ts` — never in plain text, never logged) for the given email and emails it via Resend; fails loudly (503) rather than falling back to logging the code if email isn't configured.
2. `POST /api/auth/verify-code` — every accept/reject decision (recording a wrong guess, consuming a correct one) is a single `updateMany`/`deleteMany` whose `WHERE` clause re-checks the attempt budget **and** expiry against the row's true current state at that exact statement, not a value read earlier — this is what actually closes the race an "atomic increment, then separately check/delete" approach doesn't: a correct code arriving the instant after the final wrong guess lands, but before that guess's own cleanup runs, must still be rejected, and only re-validating the budget/expiry at write-time (not read-time) guarantees that regardless of which operation's statement Postgres commits first. Also atomically single-use (two concurrent requests with the same correct code can't both mint a session). Exhausted/expired/already-consumed always returns the identical response regardless of whether the guess that triggered it was actually correct — the response text itself must not become an oracle for "was my last guess right". Creates a `Session` row and sets an httpOnly `tally_session` cookie. Both routes also apply a low, per-IP sliding-window rate limit (`src/lib/rateLimit.ts`).
...
- **Field-level encryption** (`src/lib/crypto.ts`): sensitive `Account` fields are encrypted with **AES-256-GCM** before being written to the database — a random 12-byte IV per value, with the GCM auth tag appended so tampering/corruption is detectable on decrypt. Stored format: `base64(iv):base64(authTag):base64(ciphertext)`.
  ...
  - `npm run rotate-key` (`scripts/rotate-encryption-key.ts`) re-encrypts all 8 sensitive Account fields under a new key, plus every `DatabaseBackup`'s embedded account ciphertext (otherwise a pre-rotation snapshot becomes permanently unreadable once the old key is gone). A value is only skipped once confirmed to be `isEncryptedWithKey()` the *destination* key specifically — not merely "in the current string format" (`isCurrentKeyVersion()`), which only identifies the format, not which key produced it, and would otherwise treat everything already on the current format as done regardless of which key encrypted it.
...
| `income` | CRUD for income sources; marking `isReceivedThisCycle` true logs a real `Transfer` at the actual received amount/date when supplied (`receivedAmount`/`receivedDate`), falling back to the record's usual amount/today otherwise — marking it back false deletes that month's linked Transfer(s) so a correction doesn't double-count |
...
| `trails`, `trails/[id]` | Money trails — create/list/rename/delete a named chain of transfers; `PATCH` also adds/removes member transfers (sets/clears `Transfer.trailId`) |
| `projects`, `projects/[id]`, `projects/[id]/items`, `projects/[id]/items/[itemId]` | Home projects (Planned tab) — CRUD for `Project` / `ProjectItem`; the item `PATCH` also takes `addLinks` / `removeLinkIds` to link line items to real `Expense`/`Transfer` rows (`ProjectItemLink`, optional `amountOverride`). Actuals computed client-side; deleting a project leaves the linked ledger rows untouched |
| `categories`, `categories/[id]`, `categories/reorder` | CRUD for household categories (custom, plus rename/recolour overrides for built-ins via `builtinKey` rows); `reorder` takes an ordered id list and sets `sortOrder` across the combined list |
...
| `statements/[id]/undo` | `GET` previews (record counts), `POST` executes — reverses an import as a unit, deleting every `Expense`/`Transfer`/`Income` it created plus the import itself, distinct from the plain delete route's keep-what's-logged behavior |
| `statements/activity` | Newest-first feed of every resolved statement row across **all** imports (derived from `StatementTransaction` — no separate log table), so a mistake weeks ago can be found and undone without knowing which statement it came from; "undo" there is the existing per-row `reset` action |
| `statements/extract` | AI extraction of a PDF/photo statement into transaction rows + account info |
| `statements/[id]/transactions`, `statements/[id]/transactions/[txId]/resolve` | List/manage statement rows and resolve them (confirm match, add as expense or transfer with an optional note, **link to income** for a money-in row, ignore, rename merchant) |
...
| `admin/backup` | Full household snapshot/restore (admin) — every household-scoped table (see §2 data model note), with cross-reference remapping on restore |
...
- **Income tracking** — recurring/one-off income with a `nextPayDate` that auto-rolls forward, linked to a deposit account; marking one received captures the actual amount and date (not just the usual figure), so a fluctuating income (e.g. salary) reconciles against what it really was — every monthly income total (Overview, Insights) uses that real figure for the current month when it exists, the usual estimate otherwise (`getIncomeMonthlyContribution()`).
...
- **Money trails** — an optional named grouping of transfers (`MoneyTrail` + `Transfer.trailId`, `onDelete: SetNull` so deleting a trail only un-groups) that follow the same money across several hops — income landing → sweep to savings → moved back → pay the credit card — rendered as a path with a "where the money sat, and for how long" breakdown derived from adjacent hops sharing an account. Purely a lens: the member transfers still stand on their own and no total changes. `GET/POST /api/trails`, `PATCH/DELETE /api/trails/[id]` (PATCH also adds/removes member transfers).
...
- **Home projects** (Planned tab, `ProjectsSection`) — a mini project manager: a `Project` holds `ProjectItem` line items with an *estimated* cost; the *actual* is the sum of real `Expense`/`Transfer` records linked to each item via `ProjectItemLink` (cascade-deleted with the item or the linked record; optional `amountOverride` to split one payment across items). Shows per-item over/under and a project rollup of spent vs budget, with a status (planning / active / done) and target date. A lens over ledger rows that stand on their own — deleting a project never deletes an expense or transfer. In household backups from schema v4.
...
| `AUTH_SECRET` | Yes, unless `CREDENTIALS_ENCRYPTION_KEY` is set | Keys the digest sign-in codes are hashed with before storage (`src/lib/otp.ts`) — any string, e.g. `openssl rand -base64 32`. Falls back to `CREDENTIALS_ENCRYPTION_KEY` if unset; if **neither** is set, code issuance fails closed (503) rather than falling back to a hardcoded secret — a secret known from reading the source provides no real protection |
